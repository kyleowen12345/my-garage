...
const makeProduct=(productName,price,productStocks,description,storeName,storeOwner,token,openChildred,message)=>async(dispatch)=>{
    ...
    try {
        const {data}=await axios.post(`${process.env.REACT_APP_API_KEY}/createproduct`,{
...
const getAllPInS=(storeName)=>async(dispatch)=>{
    ...
    try {
        const {data}=await axios.post(`${process.env.REACT_APP_API_KEY}/storeproduct`,{storeName})
...
const updateProductInfo=(productName,price,productStocks,description,storeName,_id,token,onClose,message)=>async(dispatch)=>{
    ...
    try {
        const {data}=await axios.post(`${process.env.REACT_APP_API_KEY}/updateproduct`,{
...
const makeProductImage=(_id,image,token,onClose,message)=>async(dispatch)=>{
    dispatch({ type: UPDATE_IMAGE_REQUEST });
    try {
        const {data}=await axios.post(`${process.env.REACT_APP_API_KEY}/productimage`,{
            _id,image
        },{
            headers: {
                Authorization: `Bearer${token}`,
