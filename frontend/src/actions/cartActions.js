...
const addtocartact=(productId,token,message,name)=>async(dispatch)=>{
  ...
  try {
    const {data}=await axios.post(`${process.env.REACT_APP_API_KEY}/addtocart`,{productId},{
        headers: {
...
const viewCart=(token)=>async(dispatch)=>{
    ...
    try {
        const {data}=await axios.get(`${process.env.REACT_APP_API_KEY}/getCartInfo`,{
            headers: {
                Authorization: `Bearer${token}`,
            },
          })
...
const deleteItemFromCart=(productId,token,name,message)=>async(dispatch)=>{
  ...
  try {
    const {data}=await axios.post(`${process.env.REACT_APP_API_KEY}/removeitem`,{productId},{
      headers: {
          Authorization: `Bearer${token}`,
      },
    })
...
const buyCart=(variables,token,message)=>async(dispatch)=>{
  ...
  try {
    const {data}=await  axios.post(`${process.env.REACT_APP_API_KEY}/successBuy`,variables,{headers: {
      Authorization: `Bearer${token}`,
  },})
...
const cartHistory=(token)=>async(dispatch)=>{
  ...
  try {
    const {data}=await axios.get(`${process.env.REACT_APP_API_KEY}/PurchasedItems`,{headers: {
      Authorization: `Bearer${token}`,
  }})
